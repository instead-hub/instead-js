...
portrait = obj {
	...
	act = 'Гм... На портретах одно и то же лицо! Улыбающееся холодной улыбкой лицо человека, лет сорока, с пустым, ничего не выражающим взглядом.',
};
...
moika = room {
	nam = 'мойка',
	enter = function()
		return cat('Я отношу грязную посуду в мойку.^^', walk('kitchen')), false;
	end
};
...
пластика — Ну, до встречи!... — Уххх...]],[[inv():add('invite');return walk('eating');]]), 
	}
};
kitchen = room {
	nam = 'столовая',
...
stolcorridor = room {
	...
	act = function(s, w)
		if w == 1 then
			return 'Да, эти люди пришли затем, чтобы поесть...';
		end
	end,
...
-- here we go!
	...
	used = function(s, w)
		...
		if w == 'trap' then
			...
			inv():del('trap');
			vent._trap = true;
			return 'Я встаю на стол и устанавливаю капкан-мышеловку на край отверстия... Мне не приходится долго ждать... Лязг железа и визг крысы красноречиво говорят о том, что дело сделано!';
		end
	end,
	obj = {
		'resh'
...
hall42 = room {
	...
	enter = function(s, f)
		...
		if not me()._brit or not galstuk._wear then
			return cat(
'Я захожу в зал, когда меня останавливает человек в форме с надписью <<ОХРАНА>>. В его руках помповое ружье.^^', walk('facectrl')), false;
		end
		facectrl:poff(1, 5);
		facectrl:pon(6);
