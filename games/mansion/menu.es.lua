...
mode_exam = action('INSPECCIONAR')
mode_take = action('TOMAR');
mode_use = action('USAR');
...
obj = stead.hook(obj, function(f, v, ...)
	...
	v.act = function(s)
		...
		if mode_exam.state then
			if not s.exam then
				return 'No hay nada inusual. '
			end
			return call(s, 'exam');
		elseif mode_take.state then

			if not s.take then
				return 'Es imposible cogerlo. '
			end
